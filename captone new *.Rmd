...
```
```{r}
### Adding columns for date,month,day,yea,day_of_week
...
```
```{r}
### Analyze all_trips_v2 by tupe and weekday
all_trips_v2 %>% 
...
```
```{r}
### Sdding a column for time of day
head(all_trips_v2)
...
```
```{r}
### Make a new colunm start_lat, start_lng, end_lat,end_lng rounded 
...
```
```{r}
### Create new dataset and remove tested drives
...
```
```{r}
### average ride length by customer in minutes
...
```
```{r}
### More members use it from 30 minutes or less
all_trips_v3 %>% 
   group_by(member_casual) %>% 
...
```
```{r}
### Average ride length of each trip by customer type in time of day
...
### Casual cuatomers are more active midmorning and afternoon
...
```
```{r}
### Members peek two times from 6am to 8am and again 4pm to 6pm
### Casual customers peak from 3pm to 6pm
all_trips_v3 %>% 
  group_by(member_casual, hour) %>% 
  summarise(count = n()) %>%  
  arrange(member_casual, hour) %>% 
...
```
```{r}
### Members are more active throughout the day and casual are more active in early night
all_trips_v3 %>% 
   group_by(member_casual) %>% 
...
### Members are active in the mornig and both members and casual customers peak in the afternoo-evening
all_trips_v3 %>% 
  group_by(member_casual, time_of_day) %>% 
  summarise(count = n()) %>% 
...
```
```{r}
### Visualiztion for total rides by user
### 
all_trips_v3 %>% 
  group_by(member_casual, time_of_day) %>% 
  summarise(count = n()) %>% 
...
```
```{r}
### Total rides by user and day of the week
...
```
```{r}
### The top start station for members is Kingsbury st location
all_trips_v3%>% 
  filter(!(is.na(start_station_name))) %>% 
  filter(member_casual== "member") %>% 
  group_by(start_station_name) %>% 
